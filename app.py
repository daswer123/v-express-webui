...
from v_express.scripts.extract_kps_sequence_and_audio import crop_face, extract_kps_sequence
...
def convert_video_gr(converter_save_path, converter_ref_img, converter_ref_img_crop, converter_ref_audio, converter_use_audio_from_video, converted_prepared_sequence, convert_video_extract, convert_video_extract_crop, convert_video_extract_device, convert_video_extract_gpu_id, convert_video_extract_insightface_model_path, convert_video_extract_height, convert_video_extract_width, convert_steps, converter_strategy, converter_reference_attention_weight, converter_audio_attention_weight, converter_seed, converter_cfg, converter_device, converter_gpu_id, converter_dtype, converter_fps, converter_context_frames, converter_context_stride, context_overlap, converter_num_pad_audio_frames, converter_standard_audio_sampling_rate, do_multi_devices_inference, save_gpu_memory):
    ...
    if convert_video_extract is not None:
       ...
       if converter_use_audio_from_video:
           converter_ref_audio = temp_audio

    # Crop reference image if needed
    if converter_ref_img_crop:
        detector = dlib.get_frontal_face_detector()
        ref_img = cv2.imread(converter_ref_img)
        gray = cv2.cvtColor(ref_img, cv2.COLOR_BGR2GRAY)
        faces = detector(gray)
        assert len(faces) == 1, f'There are {len(faces)} faces in the reference image. Only one face is supported.'
        face = faces[0]
        cropped_ref_img = crop_face(ref_img, face)
        cropped_ref_img = cv2.resize(cropped_ref_img, (convert_video_extract_width, convert_video_extract_height))
        cropped_ref_img_path = f'temp/{timestamp}_cropped_ref.jpg'
        
        # Create temp if not exists
        if not os.path.exists("temp"):
            os.mkdir("temp")
        
        cv2.imwrite(cropped_ref_img_path, cropped_ref_img)
        converter_ref_img = cropped_ref_img_path
    ...
    if do_multi_devices_inference:
        args.append('--do_multi_devices_inference')

    if save_gpu_memory:
        args.append('--save_gpu_memory')

    convert_video(args)

    # # Paste the animated face back into the original image
    # if converter_ref_img_crop:
    #     animated_video = cv2.VideoCapture(converter_save_path)
    #     original_ref_img = cv2.imread(converter_ref_img)

    #     fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    #     out = cv2.VideoWriter(converter_save_path, fourcc, converter_fps, (original_ref_img.shape[1], original_ref_img.shape[0]))

    #     while True:
    #         ret, frame = animated_video.read()
    #         if not ret:
    #             break

    #         original_ref_img[face.top():face.bottom(), face.left():face.right()] = frame
    #         out.write(original_ref_img)

    #     animated_video.release()
    #     out.release()
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("2 - Converter"):
            with gr.Row():
                with gr.Column():
                    ...
                    converter_ref_img_crop = gr.Checkbox(label="Crop the face on the image according to official guidelines", value=False)
                ...
                with gr.Column():
                    with gr.Accordion("Converter settings", open=True):
                        ...
                        converter_audio_attention_weight = gr.Slider(label="Audio Attention Weight", minimum=0.01, maximum=5, step=0.01, value=1)
                        with gr.Accordion("Extra Settings", open=False):
                            ...
                            with gr.Row():
                                ...
                                converter_dtype = gr.Dropdown(label="Dtype", choices=["bf16","fp16", "fp32"], value="fp16")
                            ...
                            gr.Markdown("### Accelerate Settings")
                            with gr.Row():
                                do_multi_devices_inference = gr.Checkbox(label="Do Multi-Device Inference", value=False)
                                save_gpu_memory = gr.Checkbox(label="Save GPU Memory", value=False)
    ...
    video_extract_model_name.change(change_extractor_path

, inputs=[video_extract_model_name], outputs=[video_extract_save_kps_path, video_extract_save_audio_path])
